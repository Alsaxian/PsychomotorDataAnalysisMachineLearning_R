#' ---
#' title: Analyse des données par méthodes d'apprentissage supervisé et non-supérvisé à travers un jeu de données psychomoteur
...
#' \begin{align} 
#' &\mathrm{Pr}(Y = 1 \mid \boldsymbol{X} = \boldsymbol{x}) \sim \mathrm{Ber}(p(\boldsymbol{x})) \quad \text{où}\\
#' 
#' & p(\boldsymbol{x}) = \dfrac{1}{1 + e^{-h(\boldsymbol{x})}} \quad \text{où}\\
#' 
#' & h(\boldsymbol{x}) = \beta^T \boldsymbol{x} + \beta_0 . \\
#' \end{align}
...
#' \[
#' \mathrm{argmin}_{\beta, \beta_0} \left\{ \sum_{i=1}^n \left[ -y_i(\beta^T \boldsymbol{x}_i + 
#' \beta_0) + \ln(1 + e^{\beta^T  \boldsymbol{x}_i + \beta_0})   \right] \right\} . 
#' \]
...
#' \left\{ \sum_{i=1}^n \left[ -y_i(\beta^T \boldsymbol{x}_i + 
#' \beta_0) + \ln(1 + e^{\beta^T  \boldsymbol{x}_i + \beta_0})   \right] + \lambda_1 \lvert \beta \rvert
...
#' \[
#' h(x) = \sum_{i=1}^N \alpha_i y_i \langle \boldsymbol{x}, \boldsymbol{x}_i \rangle + \beta_0
#' \]
#' 
...
#' \[
#' \mathrm{argmin}_{\beta, \beta_0} \left\{ \sum_{i=1}^N \max [0, 1 - y_i (\beta^T \boldsymbol{x_i} + \beta_0)]
...
#' \begin{cases}
#'     \exp \left( -\gamma \lvert \boldsymbol{x} - \boldsymbol{x'} \rvert^2\right) \quad \text{noyau gaussien} \\
#'     (\gamma \boldsymbol{x}^T \boldsymbol{x'} + c)^d \quad \text{noyau polynomial} \\
#'     \tanh (\gamma \boldsymbol{x}^T \boldsymbol{x'} + c) \quad \text{noyau sigmoïde}.
#'\end{cases}
#' \]
...
#' \[
#' p_k(\boldsymbol{x}) = \frac{1}{(2\pi)^{\frac{p}{2}} \lvert\Sigma\rvert^{\frac{1}{2}}} 
#' \exp \left\{-\frac{1}{2} (\boldsymbol{x} - \mu_k)^T \Sigma_k^{-1} (\boldsymbol{x} - \mu_k)\right\}
#' \]
#' où $p_k(\boldsymbol{x})$ représente la fonction de densité de la $k$-ième classe, $\Sigma_k$ sa matrice de 
...
#' \[
#' h_k(\boldsymbol{x}) = \boldsymbol{x}^T\Sigma^{-1}\mu_k - \frac{1}{2} \mu_k^T \Sigma^{-1}\mu_k
#' + \ln \pi_k ,
#' \]
...
#' revient à dire
#' \[
#' \Pr(Y = k \mid \boldsymbol{X} = \boldsymbol{x}) = \frac{1}{\Pr(\boldsymbol{X} = \boldsymbol{x})}
#' \Pr(Y = k) \prod_{i=1}^p \Pr(\boldsymbol{X}_i = \boldsymbol{x}_i \mid Y = k) , \, k = -1, 1
#' \]
#' ce qui entraîne un classifieur en marginalisant la loi globale de $\boldsymbol{X}$
#' \[
#' h(x) = \sum_{i=1}^p \ln\Pr(\boldsymbol{X}_i = \boldsymbol{x}_i \mid Y = 1) - 
#'  \sum_{i=1}^p \ln\Pr(\boldsymbol{X}_i = \boldsymbol{x}_i \mid Y = -1) + \ln\Pr(Y = 1) - \ln\Pr(Y = -1)
#' \]
